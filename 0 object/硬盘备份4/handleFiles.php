...
//            echo "<p style='color:red'>" . $name . "----" . $add . "</p><br />";
            $url_lists = array(
                "郑瑞蓬's 误删" =>
                    "https://pan.baidu.com/api/list?clienttype=0&app_id=250528&web=1&dp-logid=41902700202618240079&order=time&desc=1&dir=%2F%E9%83%91%E7%91%9E%E8%93%AC%27s+%E8%AF%AF%E5%88%A0",
                /*"电视剧" =>
            ...
            */
            ...
            /*相当于对当前所有已经预定义到url_lists数组中的链接进行一次访问子文件夹操作，
            并将子文件夹一同加入到url_lists数组中*/
            foreach ($url_lists as $name => $url) {
                $response = json_decode(curlHtml($url));
                $response_data = $response->list;

                foreach($response_data as $num => $everyFiles){
                    if ($everyFiles->isdir) {
                        // 解析 URL
                        ...
                        // 修改或添加 GET 参数
                        $queryParams['dir'] .= '/' . $everyFiles->server_filename; // 修改 param1 的值
...
//                        echo urldecode($modifiedUrl);
                        $url_lists[$everyFiles->server_filename] = $modifiedUrl;
                    }
                }
            }
//            print_r($url_lists);
