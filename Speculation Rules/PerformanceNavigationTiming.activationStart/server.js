        ...
        switch (_a.label) {
            case 0:
                ...
                if (req.headers.get("Sec-Purpose")) {
                    console.log("已经检测到本链接是通过prefetch获取的了，因为他设置了Sec-Purpose，值为" +
                        req.headers.get("Sec-Purpose"), paths);
                }
                file = null;
                _a.label = 1;
            case 1:
...
var handler = function (req) { return __awaiter(void 0, void 0, void 0, function () {
    var urlPattern, paths, searchParams;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                urlPattern = new URLPattern(req.url);
                paths = decodeURIComponent(urlPattern.pathname);
                searchParams = new URLSearchParams(urlPattern.search);
                if (paths === "/test") {
                    console.log("test链接获取已触发");
                    if (searchParams.has("message")) {
                        console.log("message参数为：" + searchParams.get("message"));
                    }
